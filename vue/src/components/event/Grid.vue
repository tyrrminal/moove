...
export default {
  ...
  methods: {
    sortCompare: function (a, b, key, sortDesc, formatterFn, options, locale) {
      ...
      if (key == "distance") {
        ...
        a = convertUnitValue(aa.distance.value, this.getUnitOfMeasure(aa.distance.unitOfMeasureID))
        let ba = b.activity ?? b.eventActivity;
        b = convertUnitValue(ba.distance.value, this.getUnitOfMeasure(ba.distance.unitOfMeasureID))
      }
      if (key == "speed") {
        t = "num";
        let ar = activityRate(a.activity);
        let br = activityRate(b.activity);
        a = (ar == null) ? 0 : convertUnitValue(ar.value, this.getUnitOfMeasure(ar.unitOfMeasureID));
        b = (br == null) ? 0 : convertUnitValue(br.value, this.getUnitOfMeasure(br.unitOfMeasureID));
      }
      var m;
  ...
  computed: {
    ...
    eventDistance: function () {
      ...
      let v = activities.reduce((a, c) => a + convertUnitValue(c.distance.value, this.getUnitOfMeasure(c.distance.unitOfMeasureID)), 0)
      return { value: v, units: this.getUnitsOfMeasure.find(u => u.normalUnitID == null && u.type == 'Distance') }
    },
    frPctAvg: function () {
