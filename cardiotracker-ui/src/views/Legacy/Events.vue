<template>
  <layout-default>
    ...
    <table class="table table-striped table-bordered">
      ...
      <tbody>
        <tr
          v-for="e in sortedFutureEvents"
          :key="e.event.id"
          :class="(e.registration.hasOwnProperty('registered') && !e.registration.registered) ? 'table-warning' : '' "
        >
          <td>
            <router-link
              :to="{ name: 'event', params: { id: e.event.id, user: effective_user.username }}"
            >{{ e.event.name }}</router-link>
          </td>
          ...
          <td>
            <template
              v-if="e.registration.hasOwnProperty('registered')"
            >{{ e.registration.registered ? 'Y' : 'N' }}</template>
          </td>
          <td>
            <template v-if="e.registration.fee !== null">{{ e.registration.fee | format_dollars }}</template>
          </td>
          <td>
            <template
              v-if="e.registration.hasOwnProperty('fundraising')"
            >{{ e.registration.fundraising.total | format_dollars }}/{{ e.registration.fundraising.minimum | format_dollars }}</template>
          </td>
        </tr>
      </tbody>
    </table>
        ...
        <tr v-for="e in pastEvents" class :key="e.event.id">
          ...
          <template v-if="e.hasOwnProperty('activity')">
            <template v-if="e.activity.hasOwnProperty('result')">
              ...
              <td>
                <template
                  v-if="e.activity.activity_type.description === 'Run'"
                >{{ e.activity.result.pace }}</template>
              </td>
              <td>
                <template
                  v-if="e.activity.activity_type.description !== 'Run'"
                >{{ e.activity.result.speed | format_distance }}</template>
              </td>
            </template>
            <template v-else>
...
export default {
  ...
  created() {
    ...
    this.$http.get("events/" + this.effective_user.id).then(response => {
      self.futureEvents = response.data.filter(
        e => moment(e.event.scheduled_start).diff(now) > 0
      );
      self.pastEvents = response.data.filter(
        e => moment(e.event.scheduled_start).diff(now) <= 0
      );
    });
  },
