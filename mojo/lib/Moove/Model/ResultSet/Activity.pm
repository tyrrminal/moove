...
sub before_now ($self) {
  return $self->search(
    {
      'workout.date' =>
        {'<=' => DateTime::Format::MySQL->format_datetime(DateTime->now(time_zone => 'local')->truncate(to => 'day'))}
    }, {
      join => 'workout'
    }
  );
...
sub summary ($self, $partition = undef) {    # qw(activityType baseActivityType week month year)
    ...
    $mk_ctx = sub ($record) {
      {
        activityTypes => [
          map +{id => $_},
          $self->result_source->schema->resultset("BaseActivityType")->find($record->get_column('base_activity_type_id'))
            ->activity_type_ids
        ],
  ...
  my $rs = $self->search(
    undef, {
      join   => [{activity_type => [qw(base_activity_type activity_context)]}, {activity_result => 'normalized_distance'},],
      select => [
