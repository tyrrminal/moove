...
sub in_range ($self, $start, $end) {
  my $range = [
    DateTime::Format::MySQL->format_date($start // DateTime->from_epoch(epoch => 0)),
    DateTime::Format::MySQL->format_date($end   // DateTime->new(year => 3000, month => 1, day => 1))
  ];
  $self->search(
    {
      -or => [
        ...
        {-and       => [{start_date => {'<=' => $range->[0]}}, {end_date => {'>' => $range->[1]}}]}
      ]
    }
  );
}
