  ...
  foreach my $u ($self->app->model('User')->all) {
    ...
    foreach my $act (@activities) {
      ...
      if (DateTime::Duration->compare($limit, $diff) > 0) {
        ...
        my $d = $self->app->model('Distance')
          ->find_or_create_from_miles($prev->distance->normalized_value + $act->distance->normalized_value);
        my $hr =
          defined($act->result->heart_rate && defined($prev->result->heart_rate))
        ...
        my $r = $self->app->model('Result')->create(
          {
            ...
            net_time   => $t,
            pace       => Moove::Model::Result::Result::_calculate_pace($t, $d),
            heart_rate => $hr
          }
        );
