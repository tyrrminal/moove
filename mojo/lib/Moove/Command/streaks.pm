...
sub run ($self, @args) {
  ...
  while (my $act = $activities->next) {
    ...
    if ($d > 1) {
      push(@streaks,
        {start => $ss->start_date, days => $prev->start_date->delta_days($ss->start_date)->in_units('days') + 1, type => 'streak'});
  ...
  push(
    @streaks, {
      start => $current[0]->start_date,
      days  => $current[$#current]->start_date->delta_days($current[0]->start_date)->days + 1,
      type  => 'streak'
    }
  );
  push(@streaks,
    {start => $current[$#current]->start_date, days => $t->delta_days($current[$#current]->start_date)->days - 1, type => 'gap'})
    if ($current[$#current]->start_date < $t);
